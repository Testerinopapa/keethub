-- Trivia Blitz multiplayer game room support.

-- ── Question bank (server-side, never exposed directly to clients) ──
...
GRANT SELECT ON public.trivia_rooms TO authenticated;
GRANT INSERT ON public.trivia_rooms TO authenticated;
GRANT ALL ON public.trivia_rooms TO service_role;
...
-- ── RPC: Advance question (idempotent phase stepper) ────────────────
...
BEGIN
  ...
  -- Phase: question → answer_reveal (timeout: record 0-point answers for non-responders)
  IF room_row.phase = 'question' THEN
    INSERT INTO public.trivia_answers (room_id, round_number, player_id, question_id, selected_option_id, is_correct, time_ms, points)
    SELECT
      p_room_id,
    ...
    -- Reset streaks for timeout players
    UPDATE public.trivia_players p
    SET streak = 0
    WHERE p.room_id = p_room_id
      AND p.is_connected = true
  ...
  -- Already in a phase that doesn't advance
